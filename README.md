...
Ключи внешних сервисов (OpenChargeMap, Яндекс.Карты, 2GIS) управляются не
через `.env`, а через `/admin -> Интеграции` — правки применяются без
передеплоя (кэш на бэкенде обновляется раз в минуту). Доступ только у
роли `admin`, модератор эту секцию не видит.

Первый запуск `npm run seed` сам создаёт заготовки строк для всех трёх
сервисов — останется зайти в админку, вписать ключ и включить `isEnabled`.

`OCM_API_KEY` в `.env` остаётся как запасной вариант для локальной
разработки без БД под рукой, но в проде используется значение из админки.
